    ...
    while (try it.next(io)) |entry| {
        ...
        if (device) |d| {
            if (std.mem.eql(u8, d, entry.name[0 .. entry.name.len - ".xml".len])) {
                try load_device(io, db, devices_dir, entry.name, &modules);
                return;
            }
        } else {
            try load_device(io, db, devices_dir, entry.name, &modules);
...
fn load_device(io: std.Io, db: *Database, devices_dir: std.Io.Dir, filename: []const u8, modules: *std.StringHashMap(ModuleEntry)) !void {
    ...
    while (instance_it.next()) |instance_node| {
        try load_instance(io, db, device_id, devices_dir, instance_node, modules);
    }
}

fn load_instance(io: std.Io, db: *Database, device_id: DeviceID, devices_dir: std.Io.Dir, node: xml.Node, modules: *std.StringHashMap(ModuleEntry)) !void {
